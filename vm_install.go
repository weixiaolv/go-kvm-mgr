...
func main() {
    ...
    volSys := libs.VolNameBuilder(*name, "-sys.qcow2")
    volSysPath := libs.VolNameBuilder(poolSysDir, volSys)
    volData := libs.VolNameBuilder(*name, "-data_1.qcow2")

    conn, _ := libs.GetConn(*host)
    ...
    templateVolName := libs.VolNameBuilder(*template, ".qcow2")
    ...
    // 修改替换虚拟机xml文件对应参数
    vmXmlDesc, err := libs.VMXmlHandle(*name, *cpu, *memory, *vlan, volSysPath)
    if err != nil {
